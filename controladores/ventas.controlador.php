...
class ControladorVentas{
	...
	=============================================*/
		...
		static public function ctrMostrarVentasDia(){

			$tabla = "ventas_dia";

			$respuesta = ModeloVentas::mdlMostrarVentasDia($tabla);
	...
	=============================================*/
		...
		static public function ctrMostrarOperaciones($item, $valor){
			...
			$respuesta = ModeloVentas::mdlMostrarOperaciones($tabla, $item, $valor);
	...
	=============================================*/
		...
		static public function ctrCrearVenta(){
			...
			if(isset($_POST["nuevaVenta"])){
				...
				$datosDia = array( "id_vendedor"=>$_POST["idVendedor"],
								   "id_cliente"=>$_POST["seleccionarCliente"],
								   "codigo"=>$_POST["nuevaVenta"],
								   "factura"=>$_POST["nuevaFactura"],
								   "productos"=>$_POST["listaProductos"],
				...
				$dia = ModeloVentas::mdlIngresarVentaDia($tablaDia, $datosDia);
				...
				$nuevaNotificacionVenta = $traerNotificaciones["nuevas_ventas"] + 1;

				ModeloNotificaciones::mdlActualizarNotificaciones("notificaciones", "nuevas_ventas", $nuevaNotificacionVenta);
                ...
                =============================================*/
					...
					$printer = new Printer($conector);
						...
						$tablaConfiguracion = "configuracion";
						$item = "id";
          				$valor = 1;

          				$traerConfiguracion = ControladorConfiguracion::ctrMostrarConfiguracion($item, $valor);
					...
					$printer -> feed(1); //Alimentamos el papel 1 linea

					$printer -> text($traerConfiguracion["nombre"]."\n"); // Nombre Empresa

					$printer -> text("RUC: ".$traerConfiguracion["ruc"]."\n"); // RUC Empresa

					$printer -> text("Dirección: ".$traerConfiguracion["direccion"]."\n"); // Direccion Empresa

					$printer -> text("Teléfono: ".$traerConfiguracion["telefono"]."\n"); //Telefono Empresa
					...
					$printer -> feed(1); //Alimentamos el papel 1 linea
						...
						$tablaVenta = "ventas";
						$item = "factura";
						$valor = $_POST["nuevaFactura"];

						$traerVenta = ModeloVentas::mdlMostrarVentas($tablaVenta, $item, $valor);

					if ($traerVenta["factura"] != "") {

						$printer -> text("Cliente: ".$traerCliente["nombre"]."\n"); //Nombre del Cliente

						$printer -> text("RUC Cliente: ".$traerCliente["documento"]."\n"); //Nombre del Cliente

                        /*Hacemos que el texto sea en negritas e imprimimos el nùmero de venta*/
                        $printer->setEmphasis(true);
						$printer -> text("Venta: ".$_POST["nuevaVenta"]."\n"); //Numero de la Factura
					...
					foreach ($listaProductos as $key => $value) {
						...
						$printer -> text("Gs.".number_format($value["precio"],0,",",".")." (Und x ".$value["cantidad"]. ")"." Gs.".number_format($value["total"],0,",",".")."\n");

					}

					$printer -> feed(1); //Alimentamos el papel 1 linea

					$printer -> text("NETO: Gs.".number_format($_POST["nuevoPrecioNeto"],0,",",".")."\n"); //Precio Neto de la venta

					$printer -> text("IVA (Incluído): Gs.".number_format($_POST["nuevoPrecioImpuesto"],0,",",".")."\n"); //Valor del Impuesto

					$printer -> text("-----------\n");

					$printer -> text("TOTAL: Gs.".number_format($_POST["totalVenta"],0,",",".")."\n"); //Total de la Venta
				...
				if (!empty($_POST["delivery"])) {
					...
					$datosD = array("codigo_venta" => $_POST["nuevaVenta"],
								   "conductor" => $_POST["nuevoConductor"],
								   "productos" => $_POST["listaProductos"],
								   "total" => $_POST["totalVenta"]);

					$respuestaS = ModeloCamion::mdlCrearDelivery($tablaD, $datosD);
	...
	=============================================*/
		...
		static public function ctrImprimirTicket($item, $valor){
	...
	=============================================*/
		...
		static public function ctrFacturarCliente(){

			if(isset($_POST["idfacturarCliente"])){
				...
				$item1c = "id";
				$valor1c = $_POST["facturarPlan"];

				$traerPlan = ModeloPlanes::mdlMostrarPlanes($tablaPlanes, $item1c, $valor1c);

				$item1a = "compras";
					
				$valor1a = $_POST["nuevoAporte"] + $traerCliente["compras"];
				...
				$datos = array("id_vendedor"=>$_POST["idVendedor"],
							   "id_cliente"=>$_POST["idfacturarCliente"],
							   "total"=>$traerPlan["monto"],
				...
				$respuesta = ModeloVentas::mdlFacturarVenta($tabla, $datos);
	...
	=============================================*/
		...
		static public function ctrSumaCantidadVentasDia(){

			$tabla = "ventas_dia";

			$respuesta = ModeloVentas::mdlSumaCantidadVentasDia($tabla);
